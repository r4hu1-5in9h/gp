function [post nlZ dnlZ] = infEP_shape2(hyp, mean, cov, lik, x, y)
...
while (abs(nlZ-nlZ_old) > tol && sweep < max_sweep) || sweep<min_sweep
    ...
    for i = randperm(n)       % iterate EP updates (in random order) over examples
        ...
        w_price=get_w_i_a2(x,alpha,hyp,i,1);
        w_inc=get_w_i_a2(x,alpha,hyp,i,2);
        %disp(w_price); %debug
        %disp(w_inc); %debug
        ...
        if isempty(lb) %if no solution, skip this update
            ...
            bar_tau=tau_ni;
            bar_nu=nu_ni;
            [~, dot_mu, dot_s2]=get_mom(y(i),lb,ub,hyp,bar_mu,bar_s2);
            dot_tau=1/dot_s2;
